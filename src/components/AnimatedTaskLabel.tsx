...
import Animated, {
    ...
    withTiming, withSequence, withDelay, interpolateColor
...
const AnimatedTaskLabel = memo((props: Props) => {
    ...
    const textColorAnimatedStyle = useAnimatedStyle(
        () => ({
            color: interpolateColor(textColorProgress.value,
                [0, 1],
    ...
    return (
        <Pressable onPress={onPress}>
            <AnimatedHStack alignItems='center' style={hstackAnimatedStyles}>
                <AnimatedText fontSize={19} noOfLines={1} isTruncated px={1} style={textColorAnimatedStyle}>
                    {children}
                </AnimatedText>
                <AnimatedBox position={'absolute'} borderBottomWidth={1} style={strikethroughAnimatedStyles} />
            </AnimatedHStack>
        </Pressable>
    )
})
